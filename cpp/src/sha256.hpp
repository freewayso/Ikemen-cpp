...
inline std::string sha256(const void* data, size_t n) {
  ...
  auto rotr = [](uint32_t x, uint32_t r) { return (x >> r) | (x << (32 - r)); };
  ...
  for (; i + 64 <= n; i += 64) {
    std::memcpy(block, p + i, 64);
    compress();
  }
  size_t rem = n - i;
  std::memcpy(block, p + i, rem);
  block[rem] = 0x80;
  if (rem >= 56) {
  ...
  for (int t = 0; t < 8; t++) block[63 - t] = (uint8_t)(bitlen >> (8 * t));
  compress();
  std::string out(32, '\0');
  for (int t = 0; t < 8; t++) {
    out[t * 4] = (char)(h[t] >> 24);
    out[t * 4 + 1] = (char)(h[t] >> 16);
    out[t * 4 + 2] = (char)(h[t] >> 8);
    out[t * 4 + 3] = (char)h[t];
  }
  return out;
}
